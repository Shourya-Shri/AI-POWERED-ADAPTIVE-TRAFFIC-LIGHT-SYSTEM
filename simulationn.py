...
# At top, with your other globals
timeElapsed = 0
cross_times = []       # ◀– add this line
last_tp = 0
last_aw = 0.0
stats_updated_at = -1  # track when we last updated
...
engine = pygame.mixer.Sound('sounds/engine_loop.wav')
engine.play(-1)   # loop
    ...
    def move(self):
        if(self.direction=='right'):
            if(self.crossed==0 and self.x+self.currentImage.get_rect().width>stopLines[self.direction]):   # if the image has crossed stop line now
                ...
                cross_times.append(timeElapsed)
...
def repeat():
    ...
    # Play beep right as we go yellow:
...
# Add pedestrian-related variables
pedestrianSpeeds = {'normal': 1, 'fast': 2}
pedestrianGap = 10
pedestrians = {'right': [], 'left': [], 'up': [], 'down': []}

class Pedestrian(pygame.sprite.Sprite):
    def __init__(self, direction):
        pygame.sprite.Sprite.__init__(self)
        self.direction = direction
        self.speed = pedestrianSpeeds['normal']
        self.x = x[direction][0] if direction in ['right', 'left'] else mid[direction]['x']
        self.y = y[direction][0] if direction in ['up', 'down'] else mid[direction]['y']
        self.crossed = 0
        path = "images/pedestrian.png"  # Ensure this path is correct
        self.originalImage = pygame.image.load(path)
        self.currentImage = self.originalImage  # Use currentImage to match Vehicle class
        self.rect = self.currentImage.get_rect()
        self.rect.topleft = (self.x, self.y)
        pedestrians[direction].append(self)
        simulation.add(self)
...
# Modify the main loop to handle pedestrians
...
# Add pedestrian rendering to the main loop
def renderPedestrians(screen):
    for direction in pedestrians:
        for pedestrian in pedestrians[direction]:
            screen.blit(pedestrian.currentImage, pedestrian.rect.topleft)
            pedestrian.move()

# Add emergency vehicle variables
emergencyVehicles = []

class EmergencyVehicle(pygame.sprite.Sprite):
    def __init__(self, direction):
        pygame.sprite.Sprite.__init__(self)
        self.direction = direction
        self.speed = speeds['car'] * 2  # Emergency vehicles move faster
        self.x = x[direction][0]
        self.y = y[direction][0]
        self.crossed = 0
        path = "images/ambulance.png"  # Ensure this path is correct
        self.originalImage = pygame.image.load(path)
        self.currentImage = self.originalImage  # Use currentImage to match Vehicle class
        self.rect = self.currentImage.get_rect()
        self.rect.topleft = (self.x, self.y)
        emergencyVehicles.append(self)
        simulation.add(self)
...
# Modify the main loop to handle emergency vehicles
def generateEmergencyVehicles():
    while True:
        direction = random.choice(['right', 'left', 'up', 'down'])
        EmergencyVehicle(direction)
        time.sleep(random.randint(30, 60))  # Generate emergency vehicles less frequently

# Function to render emergency vehicles
def renderEmergencyVehicles(screen):
    for vehicle in emergencyVehicles:
        screen.blit(vehicle.currentImage, vehicle.rect.topleft)
        vehicle.move()

# Add weather variables
weather = 'clear'  # Options: 'clear', 'rain', 'snow'
weatherEffects = {'clear': 1, 'rain': 0.6, 'snow': 0.3}

# Modify vehicle speeds based on weather
def updateSpeeds():
    for vehicle in simulation:
        if isinstance(vehicle, Vehicle):  # Check if the object is a Vehicle
            vehicle.speed = speeds[vehicle.vehicleClass] * weatherEffects[weather]

# Add weather rendering
def renderWeather(screen, screenWidth, screenHeight):
    if weather == 'rain':
        # Render rain effect
        for _ in range(100):
            pygame.draw.line(screen, (0, 0, 255), 
                            (random.randint(0, screenWidth), random.randint(0, screenHeight)), 
                            (random.randint(0, screenWidth), random.randint(0, screenHeight)), 1)
    elif weather == 'snow':
        # Render snow effect
        for _ in range(50):
            pygame.draw.circle(screen, (255, 255, 255), 
                            (random.randint(0, screenWidth), random.randint(0, screenHeight)), 2)

def renderWeatherText(screen, font, black, white):
    weatherText = font.render(f"Weather: {weather.capitalize()}", True, black, white)
    screen.blit(weatherText, (1100, 100))  # Display weather text at (1100, 100)
...
def changeWeather():
    ...
    while True:
        ...
        time.sleep(30)  # Change weather every 30 seconds

# Just after drawing your signals, add:

def draw_density_bar(screen, idx, x, y, width=100, height=10):
    lane = directionNumbers[idx]
    total_waiting = sum(1 for v in vehicles[lane][0]+vehicles[lane][1]+vehicles[lane][2] if v.crossed==0)
    pct = min(1.0, total_waiting/20.0)   # assume 20 vehicles = “full”
    pygame.draw.rect(screen, (50,50,50), (x,y,width,height))
    pygame.draw.rect(screen, (0,200,0), (x,y,width*pct,height))

class Main:
    thread7 = threading.Thread(name="changeWeather", target=changeWeather, args=())
    thread7.daemon = True
    thread7.start()
    ...
    thread6 = threading.Thread(name="generateEmergencyVehicles", target=generateEmergencyVehicles, args=())
    thread6.daemon = True
    thread6.start()
    ...
    # ← Insert overlay setup here:
    night_mode    = False
    night_overlay = pygame.Surface((screenWidth, screenHeight))
    night_overlay.fill((0,0,30))
    night_overlay.set_alpha(100)

    # Initialize signalTexts
    signalTexts = ["", "", "", ""]
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    time_multiplier = min(5.0, time_multiplier + 0.5)
                elif event.key == pygame.K_DOWN:
                    time_multiplier = max(0.5, time_multiplier - 0.5)
                elif event.key == pygame.K_n:          # ← toggle night mode
                    night_mode = not night_mode

        # Clear the screen
        screen.blit(background, (0, 0))
        renderWeather(screen, screenWidth, screenHeight)  # Render weather effects
        ...
        if timeElapsed > 0 and timeElapsed % 10 == 0 and stats_updated_at != timeElapsed:
        # compute throughput & avg-wait
            last_min = [t for t in cross_times if t > timeElapsed - 10]
            last_tp = len(last_min)
            last_aw = (sum(timeElapsed - t for t in last_min) / last_tp) if last_tp else 0.0
            stats_updated_at = timeElapsed    # mark as “done for this minute”


        # Render and blit
        stats_text = font.render(f"TP:{last_tp}  AW:{last_aw:.1f}s", True, white)
        screen.blit(stats_text, (1100, 80))



        renderWeatherText(screen, font, black, white)  # Display weather text
        ...
        # In your render loop, after displaying vehicle counts:
        for i in range(noOfSignals):
            draw_density_bar(screen, i,signalCoods[i][0], signalCoods[i][1]+50)
        ...
        renderEmergencyVehicles(screen)
